...
double v1x, double v1y, double v1z, double v2x, double v2y, double v2z){
	...
	*/
	...
	const double plane_dist = normalAux[X]*v0x + normalAux[Y]*v0y + normalAux[Z]*v0z;

	const double s = normalAux[X]*cen[X] + normalAux[Y]*cen[Y] + normalAux[Z]*cen[Z] - plane_dist;
...
void mesh::subtraction(const double *a, const double *b, double *c)
{
	c[0] = a[0] - b[0];
	c[1] = a[1] - b[1];
	c[2] = a[2] - b[2];
...
double mesh::squaredDist(const double *a,const double *b)
{
...
int mesh::getCellPosX(double cx){
	
	//fprintf(stderr, "\n*cell coordinate x - %f", cx);
	//fprintf(stderr, "\n*cell position x - %d", int((cx - minSpc[0])/eCells[0]));
	return int((cx - minSpc[0])/eCells[0]);
}
int mesh::getCellPosY(double cy){
	
	//fprintf(stderr, "\n*cell coordinate y - %f", cy);
	//fprintf(stderr, "\n*cell position y - %d", int((cy - minSpc[1])/eCells[1]));
	return int((cy - minSpc[1])/eCells[1]);
}
int mesh::getCellPosZ(double cz){
	
	return int((cz - minSpc[2])/eCells[2]);
}

/// https://github.com/sreiter/stl_reader
void mesh::readMeshFile(const char * meshfilename,const double dcell,const double  VminX,const double  VminY,const double  VminZ,const double  VmaxX,const double  VmaxY,const double  VmaxZ, int opt)
{	
	...
	// double minx = elem[0].node[0].pos->x;
	// double miny = elem[0].node[0].pos->y;
	// double minz = elem[0].node[0].pos->z;

	// double maxx = elem[0].node[0].pos->x;
	// double maxy = elem[0].node[0].pos->y;
	// double maxz = elem[0].node[0].pos->z;

	// for (long t = 0; t< nElements; t++){
	// 	for(int inode = 0;inode<3;inode++){
	...
	// 	}	
	// }
	initCell(dcell, VminX, VminY, VminZ, VmaxX, VmaxY, VmaxZ);		
...
// USADO EM 1
Point2 mesh::intersectionPointZ(const Point& p1, const Point& p2, const double p) {
...
bool mesh::intersectTrianglePlaneZ(const double z, std::vector<Point2> &intersectionPoints, long t) {
    int c = 0;
...
void mesh::planeInterpolationXY(const Point2 &a, const Point2 &b, const double e, std::vector<std::set<int>>& cellcoords) {
    ...
    const int p0x = static_cast<int>((a.x - minSpc[0]) *inv_e);
    const int p1x = static_cast<int>((b.x - minSpc[0]) *inv_e);
    const int p0y = static_cast<int>((a.y - minSpc[1]) *inv_e);
    const int p1y = static_cast<int>((b.y - minSpc[1]) *inv_e);
    ...
    const double dx = (b.x - a.x) / (b.y - a.y);
    const double dy = (b.y - a.y) / (b.x - a.x);
    
    if (p1x > p0x) {
        for (int_fast32_t i = p0x + 1; i <= p1x; i++) {
            // double inter = a.y - minSpc[1] + (i * e + minSpc[0] - a.x ) * dy;
            cellcoords[i].insert(static_cast<int>((a.y - minSpc[1] + (i * e + minSpc[0] - a.x ) * dy) *inv_e));
        }
    } else {
        for (int_fast32_t i = p0x; i > p1x; i--) {
            // double inter = a.y - minSpc[1] + (i * e - a.x + minSpc[0]) * dy;
            cellcoords[i-1].insert(static_cast<int>((a.y - minSpc[1] + (i * e - a.x + minSpc[0]) * dy) *inv_e));
        }
    }

    if (p1y > p0y) {
        for (int_fast32_t j = p0y + 1; j <= p1y; j++) {
            // double inter = a.x - minSpc[0] + (j * e - a.y + minSpc[1]) * dx;
            cellcoords[static_cast<int>((a.x - minSpc[0] + (j * e - a.y + minSpc[1]) * dx) *inv_e)].insert(j);
        }
    } else {
        for (int_fast32_t j = p0y; j > p1y; j--) {
            // double inter = a.x - minSpc[0] + (j * e - a.y + minSpc[1]) * dx;
            cellcoords[static_cast<int>((a.x - minSpc[0] + (j * e - a.y + minSpc[1]) * dx) *inv_e)].insert(j-1);
        }
    }
...
// LINHA PRA CELULA - 2 - RETORNA CELULA
void mesh::planeInterpolationXZ(const Point2 &a, const Point2 &b, const double e, std::vector<std::set<int>>& cellcoords) {
    
    const double inv_e = 1.0 / e; 
    // Ajustando as coordenadas dos pontos a e b para considerar o deslocamento do ponto de origem do domínio
    const int p0x = static_cast<int>((a.x - minSpc[0]) *inv_e);
    const int p1x = static_cast<int>((b.x - minSpc[0]) *inv_e);
    const int p0z = static_cast<int>((a.y - minSpc[2]) *inv_e);
    const int p1z = static_cast<int>((b.y - minSpc[2]) *inv_e);
    ...
    // Calculando os deltas de interpolação
    const double dx = (b.x - a.x) / (b.y - a.y);
    const double dz = (b.y - a.y) / (b.x - a.x);
    
    if (p1x > p0x) {
        for (int_fast32_t i = p0x + 1; i <= p1x; i++) {
            // double inter = a.y - minSpc[2] + (i * e + minSpc[0] - a.x ) * dz;
            cellcoords[i].insert(static_cast<int>((a.y - minSpc[2] + (i * e + minSpc[0] - a.x ) * dz) *inv_e));
        }
    } else {
        for (int_fast32_t i = p0x; i > p1x; i--) {
            // double inter = a.y - minSpc[2] + (i * e - a.x + minSpc[0]) * dz;
            cellcoords[i-1].insert(static_cast<int>((a.y - minSpc[2] + (i * e - a.x + minSpc[0]) * dz) *inv_e));
        }
    }

    if (p1z > p0z) {
        for (int_fast32_t j = p0z + 1; j <= p1z; j++) {
            // double inter = a.x - minSpc[0] + (j * e - a.y + minSpc[2]) * dx;
            cellcoords[static_cast<int>((a.x - minSpc[0] + (j * e - a.y + minSpc[2]) * dx) *inv_e)].insert(j);
        }
    } else {
        for (int_fast32_t j = p0z; j > p1z; j--) {
            // double inter = a.x - minSpc[0] + (j * e - a.y + minSpc[2]) * dx;
            cellcoords[static_cast<int>((a.x - minSpc[0] + (j * e - a.y + minSpc[2]) * dx) *inv_e)].insert(j-1);
        }
    }
...
// LINHA PRA CELULA - 2 - RETORNA CELULA
void mesh::planeInterpolationYZ(const Point2 &a, const Point2 &b, const double e, std::vector<std::set<int>>& cellcoords) {
    
    const double inv_e = 1.0 / e; 
    // Ajustando as coordenadas dos pontos a e b para considerar o deslocamento do ponto de origem do domínio
    const int p0y = static_cast<int>((a.x - minSpc[1]) *inv_e);
    const int p0z = static_cast<int>((a.y - minSpc[2]) *inv_e);
    const int p1y = static_cast<int>((b.x - minSpc[1]) *inv_e);
    const int p1z = static_cast<int>((b.y - minSpc[2]) *inv_e);
    ...
    // Calculando os deltas de interpolação
    const double dy = (b.x - a.x) / (b.y - a.y);
    const double dz = (b.y - a.y) / (b.x - a.x);
    
    if (p1y > p0y) {
        for (int_fast32_t i = p0y + 1; i <= p1y; i++) {
            // double inter = a.y - minSpc[2] + (i * e + minSpc[1] - a.x ) * dz;
            cellcoords[i].insert(static_cast<int>((a.y - minSpc[2] + (i * e + minSpc[1] - a.x ) * dz) *inv_e));
        }
    } else {
        for (int_fast32_t i = p0y; i > p1y; i--) {
            // double inter = a.y - minSpc[2] + (i * e - a.x + minSpc[1]) * dz;
            cellcoords[i-1].insert(static_cast<int>((a.y - minSpc[2] + (i * e - a.x + minSpc[1]) * dz) *inv_e));
        }
    }

    if (p1z > p0z) {
        for (int_fast32_t j = p0z + 1; j <= p1z; j++) {
            // double inter = a.x - minSpc[1] + (j * e - a.y + minSpc[2]) * dy;
            cellcoords[static_cast<int>((a.x - minSpc[1] + (j * e - a.y + minSpc[2]) * dy) *inv_e)].insert(j);
        }
    } else {
        for (int_fast32_t j = p0z; j > p1z; j--) {
            // double inter = a.x - minSpc[1] + (j * e - a.y + minSpc[2]) * dy;
            cellcoords[static_cast<int>((a.x - minSpc[1] + (j * e - a.y + minSpc[2]) * dy) *inv_e)].insert(j-1);
        }
    }
...
void mesh::fillBopt(){    
	...
	for (int t = 0; t<nElements;t++){
		...
		cellcd.resize(std::max({numCells[0],numCells[1],numCells[2]}));
		...
		if (int((elem[t].node[0].pos->x-minSpc[0])/eCells[0]) == int((elem[t].node[1].pos->x-minSpc[0])/eCells[0]) && int((elem[t].node[0].pos->x-minSpc[0])/eCells[0]) == int((elem[t].node[2].pos->x-minSpc[0])/eCells[0])){
			...
			int x = (elem[t].node[0].pos->x-minSpc[0])/eCells[0];
			for (int i = 0; i< cellcd.size();i++){
				if(cellcd[i].size() == 0) continue;
				// for(int_fast32_t j = *cellcd[i].begin(); j<=*cellcd[i].rbegin();++j){
				const auto [min, max] = std::minmax_element(begin(cellcd[i]), end(cellcd[i]));
				for(int_fast32_t j = *min; j<=*max;++j){
					
					for(int_fast32_t ii = x-1; ii <= x+1; ii++) for(int_fast32_t jj = i-1; jj <= i+1; jj++) for(int_fast32_t kk = j-1; kk <= j+1; kk++){
						if((ii<0) || (jj<0) || (kk<0))
							continue;
						if((ii>=numCells[0]) || (jj>=numCells[1]) || (kk>=numCells[2]))
							continue;
						 #pragma omp critical
						candidates_set[ii][jj][kk].insert(t);
		...
		if (int((elem[t].node[0].pos->y-minSpc[1])/eCells[1]) == int((elem[t].node[1].pos->y-minSpc[1])/eCells[1]) && int((elem[t].node[0].pos->y-minSpc[1])/eCells[1]) == int((elem[t].node[2].pos->y-minSpc[1])/eCells[1])){
			...
			int y = (elem[t].node[0].pos->y-minSpc[1])/eCells[1];
			for (int i = 0; i< cellcd.size();i++){
				if(cellcd[i].size() == 0) continue;
				// for(int_fast32_t j = *cellcd[i].begin(); j<=cellcd[i].back();++j){
					
				// for(int_fast32_t j = *cellcd[i].begin(); j<=*cellcd[i].rbegin();++j){
				const auto [min, max] = std::minmax_element(begin(cellcd[i]), end(cellcd[i]));
				for(int_fast32_t j = *min; j<=*max;++j){
					for(int_fast32_t ii = i-1; ii <= i+1; ii++) for(int_fast32_t jj = y-1; jj <= y+1; jj++) for(int_fast32_t kk = j-1; kk <= j+1; kk++){
						if((ii<0) || (jj<0) || (kk<0))
							continue;
						if((ii>=numCells[0]) || (jj>=numCells[1]) || (kk>=numCells[2]))
							continue;
		...
		if (int((elem[t].node[0].pos->z-minSpc[2])/eCells[2]) == int((elem[t].node[1].pos->z-minSpc[2])/eCells[2]) && int((elem[t].node[0].pos->z-minSpc[2])/eCells[2]) == int((elem[t].node[2].pos->z-minSpc[2])/eCells[2])){
			// cellcd.resize(numCells[0]);
			...
			int z = (elem[t].node[0].pos->z-minSpc[2])/eCells[2];
			for (int i = 0; i< cellcd.size();i++){
				if(cellcd[i].size() == 0) continue;
				// 
				// for(int_fast32_t j = *cellcd[i].begin(); j<=cellcd[i].back();++j){
				...
				for(int_fast32_t j = *min; j<=*max;++j){
					...
					for(int_fast32_t ii = i-1; ii <= i+1; ii++) for(int_fast32_t jj = j-1; jj <= j+1; jj++) for(int_fast32_t kk = z-1; kk <= z+1; kk++){
						if((ii<0) || (jj<0) || (kk<0))
							continue;
						if((ii>=numCells[0]) || (jj>=numCells[1]) || (kk>=numCells[2]))
							continue;
		...
		double zvert[] = {elem[t].node[0].pos->z, elem[t].node[1].pos->z, elem[t].node[2].pos->z};
		auto [min, max] = std::minmax_element(zvert,zvert+3);
		std::vector<Point2> intersections, intersectold;
		int k = int((*min-minSpc[2])/eCells[2]);

		double miny = elem[t].node[min-zvert].pos->y;
		double minx = elem[t].node[min-zvert].pos->x;

		// cellcd.resize(numCells[0]);
		intersections.reserve(2);
		intersectold.reserve(2);

		intersectTrianglePlaneZ(double((k+1)*eCells[2])+minSpc[2], intersections,t);
		...
		for (int i = 0; i< cellcd.size();i++){
			...
			const auto [min, max] = std::minmax_element(begin(cellcd[i]), end(cellcd[i]));
			for(int_fast32_t j = *min; j<=*max;++j){
				for(int_fast32_t ii = i-1; ii <= i+1; ii++) for(int_fast32_t jj = j-1; jj <= j+1; jj++) for(int_fast32_t kk = k-1; kk <= k+1; kk++){
						if((ii<0) || (jj<0) || (kk<0))
							continue;
						if((ii>=numCells[0]) || (jj>=numCells[1]) || (kk>=numCells[2]))
							continue;
		...
		// start from k+1
		for(k = k+1; k<int((*max-minSpc[2])/eCells[2]);k++){

			intersectold[0] = intersections[0]; //Salva a interseccao de z-1
			intersectold[1] = intersections[1];

			for (auto &p : cellcd) p.clear();	// Limpa o cellcd da iteracao anterior
			if (intersectTrianglePlaneZ(double((k+1)*eCells[2])+minSpc[2], intersections,t)){ // se ha interseccao do triangulo com o plano z
...
void mesh::fillBuffer(){
	...
	for (int_fast32_t t=0; t<nElements; t++){
		...
		for (int_fast32_t i = bound[0][0]; i<=bound[0][1]; i++){
			for (int_fast32_t j = bound[1][0]; j<=bound[1][1]; j++){
				for (int_fast32_t k = bound[2][0]; k<=bound[2][1]; k++){	
					if((i<0) || (j<0) || (k<0))
						continue;
			...
			for (j = 0 ; j < 2; j++){
				for (k = 0; k < ncellz; k++){	
					...
					if (nelem == 0) fprintf(stderr, "\n*cellBuffer[%d][%d][%d]-> [/] ",i,j,k);
						...
						fprintf(stderr, "\n*cellBuffer[%d][%d][%d]",i,j,k);
						for(int_fast32_t l = 0; l<nelem;l++){
...
void mesh::fillCandidates(){
	...
	for(int_fast32_t i =0; i < numCells[0]; i++){
		for(int_fast32_t j = 0; j < numCells[1]; j++){
			for(int_fast32_t k = 0; k < numCells[2]; k++){
				findCandidates(i,j,k,numCells[0],numCells[1],numCells[2]);
			}
		}
		
	}
}
void mesh::findCandidates(int cx, int cy, int cz, int ncx, int ncy, int ncz){

	//if (cx==0 && cy == 15 && cz == 3) fprintf(stderr, "\n * ID = %ld \n",id);
	int startX = std::max(cx - 1, 0);
	int startY = std::max(cy - 1, 0);
	int startZ = std::max(cz - 1, 0);
	int endX = std::min(cx + 1, ncx - 1);
	int endY = std::min(cy + 1, ncy - 1);
	int endZ = std::min(cz + 1, ncz - 1);

	for (int_fast32_t i = startX; i <= endX; ++i) {
		for (int_fast32_t j = startY; j <= endY; ++j) {
			for (int_fast32_t k = startZ; k <= endZ; ++k) {
				
				for (long w = 0; w < nElements; w++){
					if (cellBuffer[i][j][k].size() == w) break;
					candidates_set[cx][cy][cz].insert(cellBuffer[i][j][k][w]);
				}
    ...
    long *idNearMesh, long *idNearElement, double *x, double *y, double *z, double *mx, double *my, double *mz, cellGrid *grid)
{
	...
	double elapsedTime;
	int idElement[nPart];
	for(int_fast32_t i=0; i<nPart; i++)
	{
		idElement[i] = 0;
	}
	
	for(int_fast32_t i = 0; i<=2; i++) old[i] = -1;
	/// start timer
...
double mesh::calcNearestPoint(const double  *a, const double  *b, const double  *c, const double  *p, double *nearest){
	/// Real-time collision detection, Ericson, Chapter 5 - Pg 141 - function ClosestPtPointTriangle
	...
	*/
	...
	if (d1 <= 0.0 && d2 <= 0.0){
		...
		for(int_fast32_t i = 0; i<=2; i++) nearest[i]=a[i];
	...
	*/
	...
	if (d3 >= 0.0 && d4 <= d3){
		//if (flag == 1) fprintf(stderr, "\n *2 - b vertex");
		for(int_fast32_t i = 0; i<=2; i++) nearest[i]=b[i];
		return squaredDist(nearest,p);
	} 
	...
	if (vc <= 0.0 && d1 >= 0.0 && d3 <= 0.0) {
		...
		for(int_fast32_t i = 0; i<=2; i++) nearest[i]= a[i] + v * ab[i];
		return squaredDist(nearest,p);
	}
	...
	if  (d6 >= 0.0 && d5 <= d6){
		//if (flag == 1) fprintf(stderr, "\n *4 - c vertex");
		for(int_fast32_t i = 0; i<=2; i++) nearest[i]=c[i];
		return squaredDist(nearest,p);
	} 
	...
	if (vb <= 0.0 && d2 >= 0.0 && d6 <= 0.0) {
		...
		for( int_fast32_t  i = 0; i<=2; i++) nearest[i] = a[i] + w * ac[i];
		return squaredDist(nearest,p);
	}
	...
	if (va <= 0.0 && (d4 - d3) >= 0.0 && (d5 - d6) >= 0.0) {
		...
		for( int_fast32_t  i = 0; i<=2; i++) nearest[i] = b[i] + w * (c[i] - b[i]); 
		return squaredDist(nearest,p);
	}
	...
	//if (flag == 1) fprintf(stderr, "\n *7 - triangle");
	for( int_fast32_t  i = 0; i<=2; i++) nearest[i] = a[i] + ab[i] * v + ac[i] * w;
	return squaredDist(nearest,p);
...
void mesh::writeCellBufferToFile(const std::string& filename) {
    ...
    for (int_fast32_t x = 0; x < cellBuffer.size(); ++x) {
        for (int_fast32_t y = 0; y < cellBuffer[x].size(); ++y) {
            for (int_fast32_t z = 0; z < cellBuffer[x][y].size(); ++z) {
                for (int_fast32_t w = 0; w < cellBuffer[x][y][z].size(); ++w) {
