...
void mesh::readMeshFile(const char * meshfilename,const double dcell,const double  VminX,const double  VminY,const double  VminZ,const double  VmaxX,const double  VmaxY,const double  VmaxZ)
{	
	...
	for(int i =0; i < numCells[0]; i++){
		for(int j = 0; j < numCells[1]; j++){
			for(int k = 0; k < numCells[2]; k++){
				...
				//	 findCandidates(i,j,k,numCells[0],nc1,numCells[2],grid,2);
				//}
			}
		}
